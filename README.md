...
### 💴 [割 `wari`](./works/wari/) — *an event ledger that settles itself*
**五番目の実用品**。イベントの会計を **計画 → 事前 → 当日 → 事後** のフェーズで**表**に積み、最後の
**精算**タブで立て替えを**最小の送金回数**に再分配する。各行は「項目・払った人・金額」で一目、タップで
直せて**メモ欄**つき。払った人が**未定の行は計画＝予定**として表に残り**精算には入らない**。**人ごとの
傾斜**（子ども 0.5・幹事多め）を一度決めれば全出費に効き、端数は**公平に 1 円ずつ**配って合計はぴったり。
**領収書の写真**を行に添え、**送金を一件ずつ「済」**にできる。残高（立て替え−負担）は必ず合計ゼロ、
**最大の貸しと借りの相殺**でまとめると送金は**人数−1 回以下**、輪になった貸し借りは送金ゼロで消える。
割り勘アプリと違い**登録なし・サーバーなし・お金も領収書も端末から出ない**——リンク一本（金額の
計算だけ）で渡せる。コアは DOM 非依存・**13 tests**。
