...
なる？」に答える。出費ごとに**割る人**を選べ（不参加者は負担なし）、**人ごとの傾斜**（子ども 0.5・
幹事多め）を一度決めれば全出費に効き、割り切れない端数は**公平に 1 円ずつ**配って合計はぴったり。
**領収書の写真**を出費に添え、**送金を一件ずつ「済」**にできる。残高（立て替え−負担）は必ず合計
ゼロ、それを**最大の貸しと借りの相殺**でまとめると送金は**人数−1 回以下**、輪になった貸し借りは
送金ゼロで消える。割り勘アプリと違い**登録なし・サーバーなし・お金も領収書も端末から出ない**
——リンク一本（金額の計算だけ）で渡せる。コアは DOM 非依存・**12 tests**。
