...
test('センサのことば：NaN は向きではない（毒を通さない）', () => {
  assert.equal(headingFromEvent({ alpha: NaN, absolute: true }), null);
  assert.equal(headingFromEvent({ webkitCompassHeading: NaN }), null);
  // webkit が NaN でも、まともな absolute alpha があればそちらを使う
  assert.equal(headingFromEvent({ webkitCompassHeading: NaN, alpha: 90, absolute: true }), 270);
});

test('向きの川下り：NaN を飲んでも汚染されない', () => {
  const s = new HeadingSmoother(0.5);
  s.push(10);
  const before = s.value();
  assert.equal(s.push(NaN), before);          // 無視して、いまの値を返す
  s.push(10);
  assert.ok(Number.isFinite(s.value()), '値が数のまま');
});
...
test('向きの出どころ：NaN の磁石は信じない', () => {
  const now = 10_000;
  assert.equal(pickHeading({ compass: NaN, compassAt: now, now }).source, 'none');
  const p = pickHeading({ compass: NaN, compassAt: now, gps: 90, gpsSpeed: 2, gpsAt: now, now });
  assert.equal(p.source, 'gps');
});
