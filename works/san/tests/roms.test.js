...
test('棚には 5 本、すべて asm', () => {
  assert.equal(ROMS.length, 5);
  for (const r of ROMS) {
    assert.equal(r.lang, 'asm');
...
test('roms.js の src は roms/*.s と一字違わない', async () => {
  ...
  const files = { boot: '起動.s', hotaru: '蛍.s', hebi: 'へび.s', mangekyou: '万華鏡.s', dentaku: '電卓.s' };
  for (const rom of ROMS) {
