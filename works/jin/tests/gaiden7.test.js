...
import { EXTRA_GAIDEN6 } from '../js/core/gaiden_extra6.js';

const BIOMES = new Set(['green', 'desert', 'snow', 'ruins', 'volcano']);
const SIZES = new Set(['small', 'medium', 'large']);
const OBJS = new Set(['rout', 'defeat_boss', 'seize']);

test('外伝・其の七：8つの新シナリオが本編の外伝群へ合流', () => {
  assert.equal(EXTRA_GAIDEN6.length, 8);
  const ids = new Set();
  for (const s of EXTRA_GAIDEN6) {
    assert.ok(GAIDEN.includes(s) && gaidenById(s.id) === s, `${s.id} が合流`);
    assert.ok(!ids.has(s.id), `id 一意 ${s.id}`); ids.add(s.id);
  }
  assert.ok(GAIDEN.length >= 53, `外伝が53以上（${GAIDEN.length}）`);
});

test('外伝・其の七：題・物語・実在のボス職と妥当な目標を持つ', () => {
  for (const s of EXTRA_GAIDEN6) {
...
test('外伝・其の七：目標の配分が整い、魔物戦もある', () => {
  const by = o => EXTRA_GAIDEN6.filter(s => s.objective === o).length;
  assert.ok(by('defeat_boss') >= 3 && by('seize') >= 2 && by('rout') >= 1);
  assert.ok(EXTRA_GAIDEN6.filter(s => s.monster).length >= 2);
});

test('外伝・其の七：新シナリオも布陣でき必ず決着する', () => {
  for (const s of EXTRA_GAIDEN6) {
