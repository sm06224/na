...
import { Game, loadGame } from '../core/game.js';
...
function redraw() {
  ...
  if ((game.state === 'dead' || game.state === 'won') && mode !== 'dead') {
    mode = 'dead';
    try { recordScore(game); clearSave(); } catch (e) { /* localStorage 不可なら諦める */ }
...
function save() { try { if (game && game.state === 'play') localStorage.setItem(SAVEKEY, JSON.stringify(game.serialize())); } catch (e) { /* 容量超過は諦める */ } }
function clearSave() { try { localStorage.removeItem(SAVEKEY); } catch (e) {} }
function hasSave() { try { const o = JSON.parse(localStorage.getItem(SAVEKEY)); return o && o.state === 'play'; } catch { return false; } }
function resume() {
  try {
    const obj = JSON.parse(localStorage.getItem(SAVEKEY));
    game = loadGame(obj);
    setHash(game.seedRaw);
    mode = 'play'; cursor = null; exEl.hidden = true;
    screens.hide(); $('title').hidden = true;
    renderer.fit(); prev = snap(); redraw();
  } catch (e) { newRun(); }
}
function newRun() { mode = 'title'; $('title').hidden = false; screens.hide(); $('seedin').value = ''; refreshScores(); refreshResume(); $('seedin').focus(); }
function refreshResume() { const b = $('btnResume'); if (b) b.hidden = !hasSave(); }
