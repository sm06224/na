...
export class Screens {
  ...
  guide() {
    const sec = (t, lines) => `<div class="cat">${t}</div>` + lines.map(l => `<div class="ptext">${esc(l)}</div>`).join('');
    const html =
      sec('窟とは', [
        '種ひとつから、洞窟も魔物も宝も生まれる迷宮。',
        '第 15 階に眠る「窟の護符」を取り、上り階段から地上へ戻れば生還。',
        '同じ種からは、一マスもちがわない同じ窟。#s=種 で人に渡せる。',
      ]) +
      sec('死は終わり', [
        '倒れたら、その潜行は終わり（ローグライク）。やり直しはきかない。',
        'けれど種は残り、過ぎた潜行は表紙に記録される。何度でも挑める。',
      ]) +
      sec('立ち回り', [
        '・狭い通路に退き、群れを一体ずつ捌く。',
        '・傷ついたら R で休む（魔物が見えたら休めない）。',
        '・未鑑定の品は賭け。投げて敵に試す手もある。',
        '・呪われた装備は外せない。解呪の巻物を探そう。',
        '・気力（focus）は数手で戻る。技は要所で。',
        '・店では p で買う。盗めば店主が牙を剥く。',
      ]) +
      sec('地形', [
        '溶岩は踏めば焼ける。奈落に落ちれば一つ下の階へ。',
        '水・氷・蜘蛛の巣・瓦礫は足を取る。浮遊の薬で越えられる。',
        '隠し扉や罠は s で調べ、罠は D で外せる。',
      ]) +
      sec('型', [
        '戦士＝前へ出て薙ぐ／魔術師＝火と氷／盗賊＝影と急所／',
        '狩人＝遠射と立て直し／僧侶＝癒しと退魔。',
      ]);
    this.panel('窟の手引き', html, 'Escで閉じる');
    this.mode = 'msg';
  }
